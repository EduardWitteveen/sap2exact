...
namespace access2exact
{
    public class Poco2Xml
    {
        ...
        XmlElement CreateXmlRootArtikelElement(poco.BaseArtikel artikel)
        {
            ...
            issalesitem.AppendChild(xmldocument.CreateTextNode(artikel.GetType() == typeof(poco.EindArtikel) ? "1" : "0"));
            ...
            ispurchaseitem.AppendChild(xmldocument.CreateTextNode(artikel.GetType() == typeof(poco.GrondstofArtikel) || artikel.GetType() == typeof(poco.VerpakkingsArtikel)  ? "1" : "0"));
            ...
            //isassembleditem.AppendChild(xmldocument.CreateTextNode(typeof(poco.BaseSamengesteldArtikel).IsAssignableFrom(artikel.GetType()) ? "1" : "0"));
            string isAssembled = (artikel.GetType() == typeof(poco.EindArtikel) ||  artikel.GetType() == typeof(poco.ReceptuurArtikel))  ? "1" : "0";
        ...
        private XmlNode CreateXmlSalesElement(poco.BaseArtikel artikel)
        {
            var sales = xmldocument.CreateElement("Sales");
            if (artikel.GetType() == typeof(poco.EindArtikel))
            {
                poco.EindArtikel eartikel = (poco.EindArtikel)artikel;
                var price = xmldocument.CreateElement("Price");
                var currency = xmldocument.CreateElement("Currency");
                currency.SetAttribute("code", "EUR");
                price.AppendChild(currency);
        ...
        XmlElement AddXmlSamengesteldArtikel(XmlElement item, poco.BaseSamengesteldArtikel artikel)
        {
            if (typeof(poco.EindArtikel) == artikel.GetType())
            {
                var statistical = xmldocument.CreateElement("Statistical");
                var number = xmldocument.CreateElement("Number");
                number.AppendChild(xmldocument.CreateTextNode(((poco.EindArtikel)artikel).Intrastat));
            ...
            foreach(poco.Stuklijst stuklijst in artikel.Stuklijsten) {
                ...
                foreach (poco.StuklijstRegel receptuurregel in stuklijst.StuklijstRegels)
                {
        ...
        XmlElement AddXmlRootArtikelFooter(XmlElement item, poco.BaseArtikel artikel)
        {
            // leverancier info
            ...
            string leverancierstekst = "Samengesteld: Steensma";
            if (artikel.GetType() == typeof(poco.EindArtikel))
            {
                leverancierscode = "040012";
                leverancierstekst = "Eindartikel: DAMCO";
            }
            else if (artikel.GetType() == typeof(poco.GrondstofArtikel))
            {
                leverancierscode = "999980";
                leverancierstekst = "Grondstof: Onbekend";
            }
            ...
            salesunit.AppendChild(xmldocument.CreateTextNode(Convert.ToString(artikel.VerkoopGewichtNetto)));
        ...
        void ExportArtikelen(poco.ExportData data)
        {
            foreach (poco.GrondstofArtikel grondstofartikel in data.grondstofartikelen.Values)
            {
            ...
            foreach (poco.VerpakkingsArtikel verpakkingartikel in data.verpakkingsartikelen.Values)
            {
            ...
            foreach (poco.ReceptuurArtikel receptuurartikel in data.receptuurartikelen.Values)
            {
            ...
            foreach (poco.EindArtikel eindartikel in data.eindartikelen.Values)
            {
        ...
        public void WriteData(poco.ExportData data)
        {
