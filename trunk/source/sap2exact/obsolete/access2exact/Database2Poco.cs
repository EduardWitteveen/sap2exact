...
{
    public class Database2Poco
    {
        private poco.ExportData data;
        /*
        ...
        */
        ...
        public Database2Poco(MaxDB.Data.MaxDBConnection sapconnection)
        {
        ...
        public poco.ExportData ReadAllArtikelData()
        {
            data = new poco.ExportData();
        ...
        private poco.BaseArtikel ReadArtikelData(int mandt, string matnr, int ident)
        {
            poco.BaseArtikel artikel;
            ...
            {
                case "FERT":
                    var eartikel = new poco.EindArtikel();
                    ...
                    break;
                case "HALB":
                    artikel = new poco.ReceptuurArtikel();
                    break;
                case "ZROH":
                case "ROH":
                    artikel = new poco.GrondstofArtikel();
                    break;
                case "INGR":
                    artikel = new poco.IngredientArtikel();
                    break;
                case "VERP":
                    artikel = new poco.VerpakkingsArtikel();
                    break;
                default:
            ...
            if (artikel.GetType() != typeof(poco.IngredientArtikel))
            {
                /*
                var descriptionsql = @"
            ...
            artikel.PrijsGewichtNetto = artikel.GetType() == typeof(poco.VerpakkingsArtikel) ? 0.0 : 1.0;
            ...
            artikel.VerkoopGewichtNetto = Convert.ToDouble(artikelrow["NTGEW"]);
            artikel.VerkoopGewichtBruto = Convert.ToDouble(artikelrow["BRGEW"]);
            ...
            #region child data
            if (typeof(poco.BaseSamengesteldArtikel).IsAssignableFrom(artikel.GetType())) {
                artikel = ReadChildData(mandt, (poco.BaseSamengesteldArtikel) artikel, ident);
            }
        ...
        private poco.BaseSamengesteldArtikel ReadChildData(int  mandt, poco.BaseSamengesteldArtikel artikel, int ident)
        {
            var bomsql = @"
            ...
            foreach (DataRow bomrow in bomtable.Rows)
            {
                ...
                if (matnr.Length > 0)
                {
                    var receptuurregel = new poco.StuklijstRegel();
                    ...
                    poco.BaseArtikel childartikel = ReadArtikelData(mandt, matnr, ident + 1);
                    // geen ingredienten meenemen!
                    if (childartikel.GetType() != typeof(poco.IngredientArtikel))
                    {
            ...
            // TODO: niet in SAP?
            foreach(poco.Stuklijst sl in artikel.Stuklijsten) {
                sl.StuklijstTotaalAantal = 0;
                foreach(poco.StuklijstRegel slr in sl.StuklijstRegels) {
